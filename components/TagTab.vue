...
const tagClass = computed(() => {
  return {
    'tag-bg transition hover:tag-bg-hover hover:tag-shadow': !props.active,
    'tag-shadow bg-zing-700': props.active
  }
});
...
<template>
  <HeadlessTab as="a" :href="'#' + label.toLowerCase()" class="flex flex-row items-center justify-center flex-nowrap gap-[8px] py-[8px] px-[16px] overflow-hidden group relative transition rounded-md outline-none" :class="tagClass">
    <template v-if="icon == 'tabler:brand-framer'">
      <svg xmlns="http://www.w3.org/2000/svg" class="size-[18px]" :class="iconClass" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
        <path d="M6 15h12l-12 -12h12v6h-12v6l6 6v-6" />
      </svg>
    </template>
    <template v-if="icon == 'tabler:brand-nuxt'">
      <svg xmlns="http://www.w3.org/2000/svg" class="size-[18px]" :class="iconClass" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
    ...
    <template v-if="icon == 'tabler:brand-figma'">
      <svg xmlns="http://www.w3.org/2000/svg" class="size-[18px]" :class="iconClass" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
